...
!----------------------------
         function grhode(a) ! 8piG rho_dark_energy
           ...
           REAL(dl), DIMENSION(nstep_D+1) :: x, y, y2
!real :: t1, t2
!call cpu_time(t1)
...
!              print*, a, aa
                 dummy = 0.
                 CALL mysplint(x, y, y2, nstep_D+1, aa, dummy)
              ENDIF

              grhode = dummy
!              print*, '-->', aa, a, dummy, grhode
!              stop
           ENDIF
!call cpu_time(t2)
!print*, 'grhode', t2-t1
...
! ---------------------------
         function adia_sound_speed(a)
           ...
           adia_sound_speed = grhol/grhoctilde * EXP(-x)

         end function adia_sound_speed


!----------------------------
! --- Mar 2009
         SUBROUTINE Init_Rhooa(rho_a_out)
           
           USE ModelParams, ONLY: grhov, nstep_D, ailn

           IMPLICIT NONE

!         type(EvolutionVars) EV

           REAL(dl), DIMENSION(0:nstep_D,3) :: rho_a_out
           REAL(dl) :: c_D(24),w_D(1,9), tol1_D, lna, lnaend, grhoDE0
           INTEGER :: ind_D
           
! --- Inner variables
           REAL(dl) :: RhoVar, lna_i, lna_f, dlna, t1, t2
           INTEGER :: NoEqsToPropagate = 1, NoVar_D, i
...
! --- Mar 2009
       subroutine init_background
         ...
         DOUBLE PRECISION, dimension(nstep_D+1) :: x, y, y2
...
!         print*, 'pippo'
            OPEN(unit=34,file='grhoDE_lna.txt',form='unformatted',status='unknown',action='write')
            write(34) grhoDE_lna
...
!         print*, yp1, ypn
            ...
            CALL spline(x, y, nstep_D+1, yp1, ypn, y2)
            grhoDE_lna(:,3) = y2
...
!         end if
! ------ :D
          if (CP%do_affine_eos .OR. CP%do_exp_eos) then 
...
! ------ :D
        ...
        if (CP%do_exp_eos == .false.) grhov_t = grhov*a**(-1-3*w_lam)
        if (CP%do_affine_eos .OR. CP%do_exp_eos) grhov_t = grhode(a)*a2
! ---
...
! ------ :D
        if (CP%do_affine_eos ==.false.) gpres=(grhog_t+grhor_t)/3+grhov_t*w_lam
        if (CP%do_exp_eos ==.false.) gpres=(grhog_t+grhor_t)/3+grhov_t*w_lam
        if (CP%do_affine_eos .OR. CP%do_exp_eos) gpres=(grhog_t+grhor_t)/3+grhov_t*w_de(a)
! ---
...
!  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
        ...
        if (w_lam /= -1 .and. w_Perturb .and. CP%do_affine_eos==.false. .and. CP%do_exp_eos==.false.) then
           clxq=y(EV%w_ix)
           vq=y(EV%w_ix+1) 
...
!!$        write(*,*) 'initial', w_de(a)
        IF (CP%do_affine_eos .OR. CP%do_exp_eos) initv(1,i_clxq) = ( 1.+w_de(a) ) * initv(1,i_clxb)
...
! ------ :D
            if (CP%do_affine_eos == .false.) gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_lam
            if (CP%do_exp_eos == .false.) gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_lam
            if (CP%do_affine_eos .OR. CP%do_exp_eos) gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_de(a)
! ---
...
! ------ :D
        if (CP%do_affine_eos ==.false.) gpres=(grhog_t+grhor_t)/3._dl+grhov_t*w_lam 
        if (CP%do_exp_eos ==.false.) gpres=(grhog_t+grhor_t)/3._dl+grhov_t*w_lam 
        if (CP%do_affine_eos .OR. CP%do_exp_eos) gpres=(grhog_t+grhor_t)/3._dl+grhov_t*w_de(a) 
! ---
