!! May 2009 Oliver for the second quasi-exponential model.
!!
...
! --- Apr 2009 quasi-exponential model
        P%do_qexp_eos = Ini_Read_Logical('do_qexp_eos', .false.)
        if (P%do_barotropic_sos .and. P%do_qexp_eos) WRITE(*,*) " NB - evolving sound speed!"
! ---
        P%omegal = Ini_Read_Double('omegal', 0.7d0)
        P%omegatr = Ini_Read_Double('omegatr', 1.4d0)
        P%alpha = Ini_Read_Double('alpha', -1.d0)
! --- Quasi-exponential eos :-O Apr 2009
        P%Aw0 = Ini_Read_Double('Aw0', 1.732d0)
        P%omegacritic = Ini_Read_Double('omegacritic', 1.313d0)
        P%omegapinf = Ini_Read_Double('omegapinf', 0.721d0)
! --- Affine EoS
        if (P%do_barotropic_sos) then 
           if (P%do_affine_eos) cs2_lam = P%alpha
! --- Exp EoS
           if (P%do_exp_eos .OR. P%do_qexp_eos) PRINT*, "!!!- Check you modified properly the code to take into account evolving SoS"!cs2_lam = cs
           if (.not. P%do_exp_eos .AND. .not. P%do_affine_eos .AND. .not. P%do_qexp_eos) cs2_lam = w_lam
        endif
! ---
