!! May 2009 Oliver. Modification for the second variant of the quasi exponential model. 
!!
!!
...
! ------ :D
         logical  :: do_barotropic_sos, do_affine_eos, do_exp_eos, do_qexp_eos
         real(dl) :: omegal, alpha
! --- Mar 2009
         real(dl) ::omegatr
! --- Apr 2009 :-O for the quasi-exponential variant
         real(dl) :: Aw0, omegacritic, omegapinf
...
! --- Mar 2009
        real(dl) :: omegactilde, grhoctilde, gpinf
        INTEGER, PARAMETER :: nstep_D = 500
...
! --- Mar 2009
             IF (CP%do_exp_eos) THEN
                ...
                grhomdf = grhov - grhol
             ENDIF
! --- May 2009 :-O the variant quasi-exponential eos

             IF ((CP%do_qexp_eos) .and. (CP%Aw0 + gpinf/grhoctilde .ge. 1)) THEN
                print*, "Greater than 1."
                grhoctilde = grhom * CP%omegacritic
                gpinf = grhom * CP%omegapinf
                grhol = gpinf 
                grhomdf = grhov - grhol
             ENDIF

             IF ( (CP%do_qexp_eos) .and. (CP%Aw0 + gpinf/grhoctilde .lt. 1)) THEN
                print*, "Lower than 1."
                grhoctilde = grhom * CP%omegacritic
                grhol = 0.
                grhomdf = grhov - grhol
...
! ------ :D           
              if (CP%do_affine_eos) then
                ...
                if (CP%do_barotropic_sos == .false.) write(*,*) "Effective sound speed chosen"
...
!                write(*,'("Delta                = ",f8.5)') 4*p0*0. - (1+CP%alpha)**2
                if (CP%do_barotropic_sos) write(*,*) "Barotropic fluid chosen"
                if (CP%do_barotropic_sos == .false.) write(*,*) "Effective sound speed chosen" 
! --- Apr 2009 quasi-exponential EoS
             elseif (CP%do_qexp_eos) THEN
                write(*,*) ""
                write(*,*) "The second Quasi-Exponential EoS: parameters:"
                write(*,'("Aw0              = ",f13.5)') CP%Aw0
                write(*,'("omegacritic             = ",f13.5)') CP%omegacritic
                write(*,'("omegapinf             = ",f13.5)') CP%omegapinf
!                write(*,'("grhom*p0             = ",e13.5)') p0
!                write(*,'("p0                   = ",e13.5)') p0/8./pi
                write(*,'("grhom                = ",e13.5)') grhom
                write(*,'("grhov                = ",e13.5)') grhov
