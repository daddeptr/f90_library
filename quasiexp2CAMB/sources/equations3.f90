!! May 2009 Oliver
!! I modify the code for the second quasi-exponential EoS: p = -pinf + (pinf - A*rho)*exp(-rho/rhoc). The scope is to avoid the cosmological constant by deviating from the LambdaCDM model recently.
!!
...
! ------ Exponential EoS and quasi-exponential EoS
           IF (CP%do_exp_eos .OR. CP%do_qexp_eos) THEN
...
!----------------------------
         function w_de(a)
           ...
           ENDIF
!!$           write(*,*) "w_de", w_de
! --- :-O the second quasi-exponential.
           IF (CP%do_qexp_eos) THEN
              rde = grhode(a)
              w_de = (-gpinf + (gpinf - CP%Aw0 * rde) * EXP(-rde/grhoctilde))/rde
           ENDIF
...
!----------------------------
         function w_de_dot(a)
           ...
           real(dl) :: rde, x
!           real(dl), parameter :: G=6.6742e-11_dl
           external dtauda
...
!              w_de_dot = -3.*adotoa*(1.+w_de(a))*(adia_sound_speed(a)-w_de(a))
           ENDIF
! ---- the second quasi-exponential EoS :-O

           IF (CP%do_qexp_eos) THEN 
              rde = grhode(a)
              w_de_dot = (((-CP%Aw0 + (CP%Aw0 * rde / grhoctilde) - (gpinf / grhoctilde)) * EXP(-rde/grhoctilde)) - ((-gpinf + (gpinf - CP%Aw0 * rde) * EXP(-rde/grhoctilde))/rde)) * (-3.* adotoa * (rde + ( (-gpinf + (gpinf - CP%Aw0 * rde) * EXP(-rde/grhoctilde)))))
           ENDIF
              
!!$           write(*,*) "w_de_dot", w_de_dot
...
! --- :-O the second quasi-exponential
        ...
        IF (CP%do_qexp_eos) THEN
           rde = grhode(a)
           cs2_ad = (-CP%Aw0 + (CP%Aw0 * rde / grhoctilde) - (gpinf / grhoctilde)) * EXP(-rde/grhoctilde)
...
!         type(EvolutionVars) EV
           ...
           INTEGER :: ind_D
           LOGICAL, OPTIONAL, INTENT(INOUT) :: already_done! = .false.
...
! --- The second quasi-exponential :-O
           
        IF (CP%do_qexp_eos) THEN
           x = grho_i(i_grho)/grhoctilde
           dgrhodlna = -3._dl * (grho_i - gpinf  + (gpinf -  CP%Aw0 * grho_i) * EXP(-x))
        ENDIF
...
! --- Mar 2009
       subroutine init_background
         ...
         USE ModelParams, ONLY: CP, grhoDE_lna, nstep_D
...
!         print*, CP%do_exp_eos
         ...
         IF (CP%do_exp_eos .OR. CP%do_qexp_eos) THEN 
            ...
            CALL Init_Rhooa(grhoDE_lna)

!         print*, 'pippo'
            OPEN(unit=34,file='grhoDE_lna.unf',form='unformatted',status='unknown',action='write')
            write(34) grhoDE_lna
...
! Bisection subroutine for the calculation of the fixed point
! --------------------------
...
!         end if
! ------ :D
          if (CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_qexp_eos) then 
         ...
         !Not tested for non-adiabatic         
           ...
           if (.not. EV%no_rad_multpoles .and. tauend > tau_switch .and. DoLateRadTruncation &
              ...
              if (CP%Num_Nu_massive == 0) then
                 ...
                 if ( (w_lam /= -1 .and. w_Perturb) .or. CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_qexp_eos) then
...
! ------ :D
        if (CP%do_affine_eos == .false. .AND. CP%do_exp_eos == .false. .AND. CP%do_qexp_eos == .false.) grhov_t = grhov*a**(-1-3*w_lam)
        if (CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_qexp_eos) grhov_t = grhode(a)*a2
! ---
...
! ------ :D
        if (CP%do_affine_eos ==.false. .AND. CP%do_exp_eos ==.false. .AND. CP%do_qexp_eos == .false.) gpres=(grhog_t+grhor_t)/3+grhov_t*w_lam
        if (CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_qexp_eos) gpres=(grhog_t+grhor_t)/3+grhov_t*w_de(a)
! ---
...
!  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
        ...
        if ((w_lam /= -1 .and. w_Perturb) .and. CP%do_affine_eos==.false. .and. CP%do_exp_eos==.false. .AND. CP%do_qexp_eos == .false.) then
           clxq=y(EV%w_ix)
           vq=y(EV%w_ix+1) 
...
!!$        write(*,*) 'initial', w_de(a)
        IF (CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_qexp_eos) initv(1,i_clxq) = ( 1.+w_de(a) ) * initv(1,i_clxb)
...
! --- :-O the quasi-exponential model can or not cross the p=-rho line, depending on A. Therefore, we must distinguish between the two cases. The difference with the standard second exponential model is that we cannot find the fixed point analytically. We write here the approximation rhol circa pinf, supposing rhoc very small compared to rhol. Anyway we are more interested in what happens when there is no cosmological constant.
               IF (CP%do_qexp_eos .AND. (CP%Aw0 + gpinf/grhoctilde) .ge. 1) grho =  grho + (grhoc+grhob)/a + (grhode(a) - gpinf)*a**2
               IF (CP%do_qexp_eos .AND. (CP%Aw0 + gpinf/grhoctilde) .lt. 1) grho =  grho + (grhoc+grhob)/a + grhode(a) * a**2
...
!!$         WRITE(*,*) "equations.f90, fderivs; w_lam = ", w_lam
         if (CP%do_affine_eos) then 
            grhov_t = grhode(a)*a2 
         elseif (CP%do_exp_eos .OR. CP%do_qexp_eos) THEN
            grhov_t = rde * a2
         ELSE
...
!            rde = grhode(a)
            ...
            wde = (-gpinf + (gpinf - CP%Aw0 * rde) * EXP(- rde / grhoctilde)) / rde
!            Dadotoa = 1./(a*dtauda(a))
            wded = (( (-CP%Aw0 - (gpinf / grhoctilde) + (CP%Aw0 * rde / grhoctilde)) * EXP(-rde / grhoctilde)) - ( (-gpinf + (gpinf - CP%Aw0 * rde) * EXP(- rde / grhoctilde)) / rde)) * (rde + ( (-gpinf + (gpinf - CP%Aw0 * rde) * EXP(- rde / grhoctilde)))) * (-3. * adotoa)
            cad2 = (-CP%Aw0 - (gpinf / grhoctilde) + (CP%Aw0 * rde / grhoctilde)) * EXP(-rde / grhoctilde)
            IF (CP%do_barotropic_sos) cs2_lam = cad2
         ENDIF
...
! ------ :D
            if (CP%do_affine_eos == .false. .AND. CP%do_exp_eos == .false. .and. CP%do_qexp_eos == .false.) gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_lam
!            if () gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_lam

            if (CP%do_affine_eos) gpres = gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_de(a)
            if (CP%do_exp_eos .OR. CP%do_qexp_eos)    gpres = gpres+ (grhog_t+grhor_t)/3 +grhov_t*wde
! ---
...
! ------ :D
    if (CP%do_affine_eos ==.false. .AND. CP%do_exp_eos ==.false. .AND. CP%do_qexp_eos ==.false.) gpres=(grhog_t+grhor_t)/3._dl+grhov_t*w_lam 
    if (CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_qexp_eos) gpres=(grhog_t+grhor_t)/3._dl+grhov_t*w_de(a) 
! ---
...
! ------ :D
            if (CP%do_affine_eos ==.false. .AND. CP%do_exp_eos ==.false. .AND. CP%do_qexp_eos ==.false.) gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_lam
            if (CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_qexp_eos) gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_de(a)
! ---
