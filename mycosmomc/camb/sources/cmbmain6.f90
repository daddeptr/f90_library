...
!  Increased the number of Cls too
!
!
!! Dec 2009 D.Pietrobon
...
! perturbation equations, and InitialPower which provides the initial power spectrum.
  ...
  !        real(dl), dimension(:,:), pointer :: Delta_l_q
      ...
      integer SourceNum
      !SourceNum is total number sources (2 or 3 for scalars, 3 for tensors). !:D ISW sources added
...
!!$           allocate(iCl_Scalar(CTransS%ls%l0,C_Temp:C_last+2*CP%Num_ISW_catalogs,CP%InitPower%nn))
! :DP I add C_l^ISW only too
           allocate(iCl_Scalar(CTransS%ls%l0,C_Temp:C_last+2*CP%Num_ISW_catalogs+1,CP%InitPower%nn))
!ejv
! ---
           iCl_scalar = 0
...
!jv
               Num_first_ISW = 4
!!$               SourceNum = 3+CP%Num_ISW_catalogs
! --- :DP
               SourceNum = 3+CP%Num_ISW_catalogs +1
               isource_isw = SourceNum
! +1 is for ISW component only (mainly as check)
! ---
!            SourceNum=3
               C_last = C_PhiTemp
            else
...
! --- :DP
!!$               SourceNum = 3+CP%Num_ISW_catalogs
               SourceNum = 2+CP%Num_ISW_catalogs +1
               isource_isw = SourceNum
! ---
!            SourceNum=2
               C_last = C_Cross
            ...
            Num_first_ISW = 4
! --- :DP
!? if not scalar, what are you computing then? ---
!!$            SourceNum = 3+CP%Num_ISW_catalogs
            SourceNum = 3+CP%Num_ISW_catalogs +1
            isource_isw = SourceNum
! ---
...
! EV%q2=EV%q**2
         ...
         y=0
! ------ :D
! EV%q= 4.556164725096001E-004
! EV%q= 0.002
! EV%q2=EV%q**2
         call initial(EV,y, taustart)
! ------ :D
!!$         write(*,*) "cmbmain.f90:"
!!$         write(*,*) "initial sources:", y(3),y(4),y(5),y(6),y(7),y(EV%w_ix),y(EV%w_ix+1)
! ---
...
! ------ :D
   ...
   EV%q2=EV%q**2
   call initial(EV,y, taustart)
   write(*,*) 'Initial perts:', y(1),y(3),y(EV%w_ix),y(EV%w_ix+1)*EV%q
!           tol1=tol/exp(AccuracyBoost-1)
           tol1 = 1.d-6
!       call GaugeInterface_EvolveScal(EV,tau,y,CP%tau0,tol1,ind,c,w)
       open(unit=18,file='test/camb_pert_k0.005_tgh.dat', status='unknown', action='write', form='formatted')
        do j=2,3000
          tauend = taustart * exp(j/3000._dl*log(CP%tau0/taustart))
          call GaugeInterface_EvolveScal(EV,tau,y,tauend,tol1,ind,c,w)
         
             write(18,'(6e20.6)') y(1), w_de(y(1)), w_de_dot(y(1)), cs2_ad(y(1)), y(EV%w_ix), y(EV%w_ix+1)*EV%q
!!$             write(18,'(7e20.6)') y(1), y(3), y(4), y(5), y(6), y(7), y(EV%w_ix)
         end do
       close(18)
        stop
else
...
!jv modified to ensure integration up to today!!!!!
      if (.not. DebugEvolution .and. (EV%q*tauend > max_etak_scalar .and. tauend > taurend) &
                 .and. .not. CP%Dolensing .and. CP%Num_ISW_catalogs==0 .and. &
                  (.not.CP%WantTransfer.or.tau > tautf(CP%Transfer%num_redshifts))) then
!ejv            
! ---
...
! ------ :D
!!$             write(*,*) EV%k_buf
!!$             if (abs(EV%k_buf-0.05)<0.001) write(*,*) EV%k_buf
!!$             write(45,'(4e13.5)') y(1), y(EV%w_ix),y(EV%w_ix+1),y(3)
! ---
...
!!$                     if ((CP%Dolensing .or. IV%q*atau0(i) < max_etak_scalar) .and. xf > 1.e-8_dl) then
                     if ((CP%Dolensing .or. IV%q*atau0(i) < max_etak_scalar .or. CP%Num_ISW_catalogs > 0) & 
...
! --- :DP
! Maybe I should modify accordingly
!!$         if (SourceNum==3 .and. (.not. DoInt .or. UseLimber(l,IV%q))) then
         if ( (SourceNum==3 .and. (.not. CP%do_isw) .and. (.not. DoInt .or. UseLimber(l,IV%q))) .or. (CP%do_isw .and. CP%DoLensing) ) then
! ---
            !Limber approximation for small scale lensing (better than poor version of above integral)
...
!                do nISWcat= Num_first_ISW,  Num_first_ISW+ CP%Num_ISW_catalogs

                   ! gal-gal (gg)
!!$                print*, SourceNum, C_last, CP%Num_ISW_catalogs, Num_first_ISW

                   iCl_scalar(j,C_last+1:C_last+ CP%Num_ISW_catalogs,pix) = &
                        iCl_scalar(j,C_last+1:C_last+ CP%Num_ISW_catalogs,pix) +  &
                        apowers*CTrans%Delta_p_l_k( Num_first_ISW: Num_first_ISW+ CP%Num_ISW_catalogs-1,j,q_ix)**2*dlnk

                   ! gal-temp (gT)
! ---:DP
! ISW contribution only
!!$                   iCl_scalar(j, C_last+CP%Num_ISW_catalogs+1:C_last+2*CP%Num_ISW_catalogs, pix) = &
!!$                          iCl_scalar(j, C_last+CP%Num_ISW_catalogs+1:C_last+2*CP%Num_ISW_catalogs,pix) + &
!!$                          apowers*CTrans%Delta_p_l_k( Num_first_ISW: Num_first_ISW+ CP%Num_ISW_catalogs-1,j,q_ix)* &
!!$                          CTrans%Delta_p_l_k(1,j,q_ix)*dlnk

                   iCl_scalar(j, C_last+CP%Num_ISW_catalogs+1:C_last+2*CP%Num_ISW_catalogs, pix) = &
                        iCl_scalar(j, C_last+CP%Num_ISW_catalogs+1:C_last+2*CP%Num_ISW_catalogs,pix) + &
                        apowers*CTrans%Delta_p_l_k( Num_first_ISW: Num_first_ISW+ CP%Num_ISW_catalogs-1,j,q_ix)* &
                        CTrans%Delta_p_l_k(isource_isw,j,q_ix)*dlnk
! -isw olny
                   ! isw-isw (IswIsw)
                   iCl_scalar(j,C_last+2*CP%Num_ISW_catalogs+1,pix) = &
                        iCl_scalar(j,C_last+ 2*CP%Num_ISW_catalogs+1,pix) +  &
                        apowers*CTrans%Delta_p_l_k( isource_isw,j,q_ix)**2*dlnk
! ---
!                end do
             end if
...
! --- :DP
                   ! gal-temp (gT)
                   iCl_scalar(j,C_last+2* CP%Num_ISW_catalogs+1,pix) = &
                          iCl_scalar(j,C_last+2* CP%Num_ISW_catalogs+1,pix) * dbletmp
             end if
! ---
           end do
         !$OMP END PARAllEl DO
...
!!$               do i = C_Temp, C_last
!jv
!               do i = C_Temp, C_last+2*CP%Num_ISW_catalogs
!ejv
               do i = C_Temp, C_last+2*CP%Num_ISW_catalogs+1 ! isw-isw added
! ---              
