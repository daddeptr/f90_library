...
!! Modified to include ISW computation merging my cmbfast changes and Jussi Valiviita's suggestions
!!
...
!!$        if (narguments() /= 0)  call getarg(1,InputFile)
...
! ------ :D 2007 1st Dec, Davide modification
        P%do_barotropic_sos = Ini_Read_Logical('do_barotropic', .false.)
        P%do_affine_eos = Ini_Read_Logical('do_affine_eos', .false.)

! --- 2009 Dec - isw
        P%Num_ISW_catalogs = 0
        P%do_isw  = Ini_Read_Logical('do_isw', .false.)
	if (P%do_isw) then
           P%Num_ISW_catalogs = 1
           P%do_gw   = Ini_Read_Logical('do_gw', .false.)
           if (P%do_gw) then
              P%znot    = Ini_Read_Double('znot', 0.35_dl)
              P%sigz    = Ini_Read_Double('sigz', 0.15_dl)
              P%norm_gw = Ini_Read_Double('norm_gw', 1._dl)
!!$              print*, " Gaussian CDM window function selected:"
!!$              write(*,140) P%norm_gw, " exp( ( (z-", P%znot, " ) / ", P%sigz, " )^2 )"
!!$140           format(1f6.3,1a11,1f6.3,1a5,1f6.3,1a6)
!!$              print*, ""
           endif
        endif
!!$	P%Num_ISW_catalogs = Ini_Read_Int('Num_ISW_catalogs', 0)
! ---
! --- 2009 Mar - exponential models
        P%do_exp_eos = Ini_Read_Logical('do_exp_eos', .false.)
        if (P%do_barotropic_sos .and. P%do_exp_eos) WRITE(*,*) " NB - evolving sound speed!"
	P%do_tgh_eos = Ini_Read_Logical('do_tgh_eos', .false.)

! ---
        P%omegal  = Ini_Read_Double('omegal', 0.7d0)
        P%omegatr = Ini_Read_Double('omegatr', 0.1d0)
        P%alpha   = Ini_Read_Double('alpha', -1.d0)
	P%omegaf  = Ini_Read_Double('omegaf', 1.5d0)
	IF (P%omegaf < 1.d0) print*,'Bad choice for "omegaf<1"!'

! --- Affine EoS
        if (P%do_barotropic_sos) then 
           if (P%do_affine_eos) cs2_lam = P%alpha

! --- Exp EoS
!!$           if (P%do_exp_eos .OR. P%do_tgh_eos) PRINT*, "!!!- Check you modified properly the code to take into account evolving SoS"!cs2_lam = cs
           if (.not.P%do_exp_eos .AND. .not. P%do_affine_eos .AND. .not. P%do_tgh_eos) cs2_lam = w_lam
        endif
! ---
       ...
       P%InitPower%rat(:) = 1
! ------ :DP
       P%InitPower%k_0_scalar = Ini_Read_Double('scalar_pivot', 0.002_dl)
! ---
...
! ------ :DP
             P%InitPower%k_0_tensor = Ini_Read_Double('tensor_pivot', 0.002_dl)
! ---
          end if
...
call cpu_time(t2)
print*, " ---:DP Elapsed time: ", (t2-t1)/60.
