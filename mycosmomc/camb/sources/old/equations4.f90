...
! --- Mar 2009
       subroutine init_background
         ...
         USE ModelParams, ONLY: CP, grhoDE_lna, nstep_D, back_ini_status
...
!         print*, 'pippo'
            IF (CP%do_exp_eos) OPEN(unit=34,file='exp_grhoDE_lna.unf',form='unformatted',status='unknown',action='write')
...
!!$              !Tommaso writes this function and checks the minus sign etc.
              if (CP%do_gw) then
                 sources( Num_first_ISW + ISWcat - 1) = &
                      adotoa/a * clxc * CP%norm_gw * exp( -( (zz-CP%znot)/CP%sigz )**2 )
              else
                 sources( Num_first_ISW + ISWcat - 1) = &
                      adotoa/a * clxc
              endif
...
! ---
         ...
         k2   = EV%k2_buf
! ------ :D
         IF (CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_tgh_eos) clxq = y(EV%w_ix)
! ---
