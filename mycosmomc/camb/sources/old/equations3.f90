...
! ------ :D
        if (CP%do_affine_eos == .false. .AND. CP%do_exp_eos == .false. .AND. CP%do_tgh_eos == .false.) grhov_t = grhov*a**(-1-3*w_lam)
...
! ------ :D
        if (CP%do_affine_eos ==.false. .AND. CP%do_exp_eos ==.false. .AND. CP%do_tgh_eos==.false.) gpres=(grhog_t+grhor_t)/3+grhov_t*w_lam
...
!  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
        ...
        if ((w_lam /= -1 .and. w_Perturb) .and. CP%do_affine_eos==.false. .and. CP%do_exp_eos==.false. .AND. CP%do_tgh_eos==.false.) then
           clxq=y(EV%w_ix)
           vq=y(EV%w_ix+1) 
...
! ------ :D
            if (CP%do_affine_eos == .false. .AND. CP%do_exp_eos == .false. .AND. CP%do_tgh_eos==.false.) gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_lam
...
! ------ :D
    if (CP%do_affine_eos ==.false. .AND. CP%do_exp_eos ==.false. .AND. CP%do_tgh_eos==.false.) gpres=(grhog_t+grhor_t)/3._dl+grhov_t*w_lam 
