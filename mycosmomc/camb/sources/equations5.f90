!! DP CQ
!! Corrected for normalization delta: sqrt(As) in source(3)
!!
!!
!! Jan 2010 D.Pietrobon
!  Number of sources (not Cls) increased to extract the ISW contribution only.
!
!! Dec 2009 Pietrobon D.
!! Addressing the ISW project: computation of the cross-Cls.
!! According to A.Lewis
!!
!!  http://cosmocoffee.info/viewtopic.php?t=227&highlight=cross+correlation+isw+compute
!!
!! it is possible to use the lensing encoding.
!! This may be enough to extract the cross-Cls only, but it would unable lensing computation. I'd like to try to keep both following what I did in cmbfast.
!!
!! I thank Jussi Valiviita who gave me useful advices and pieces of code (ISW-LSS_changes_JV.f90)
!!
!! DERVIS NOT PROPERLY MODIFIED!!!
!!
!! Mar 2009 Pietrobon D.
!! Modified to study the exponential models together with Oliver Piattella and Marco Bruni
!!
!! Mar 2007 Pietrobon D.
!! Modified to study affine model in collaboration with M.Bruni, C.Quercellini and A.Balbi
...
!----------------------------
         function grhode(a) ! 8piG rho_dark_energy
           use precision
           use ModelParams

           implicit none

           real(dl) :: grhode, aa, dummy
           real(dl), intent(IN) :: a
...
! ------ Affine EoS
           IF (CP%do_affine_eos) THEN 
              ...
              grhode = grhol + grhomdf * aa**( -3*(1.+CP%alpha) )
           ENDIF

! ------ Exponential EoS
           IF (CP%do_exp_eos .OR. CP%do_tgh_eos) THEN

              IF (a .LT. MINVAL(EXP(grhoDE_lna(:,1)))) THEN

                 IF (a == 0.) aa = 1.d-30
                 IF (a/=0.) aa = a

                 dummy = MAXVAL(grhoDE_lna(:,2))*( aa/MINVAL(EXP(grhoDE_lna(:,1))) )**(-3)
!                 if (a < 1.d-30) print*, '       < 10^-30', aa, dummy
              ELSE
                 aa = LOG(a)
                 dummy = 0.
!                 CALL grhoDE_splint(x, y, y2, nstep_D, aa, dummy)
                 CALL grhoDE_splint(aa, dummy)
              ENDIF
...
! --- Affine
           IF (CP%do_affine_eos) w_de = CP%alpha + p0 / grhode(a)

! --- Exp
           IF (CP%do_exp_eos) THEN
              rde = grhode(a)
              x = (rde - grhol) / grhoctilde
              w_de = - grhol * EXP(-x) / rde
           ENDIF

! --- Tgh
           IF (CP%do_tgh_eos) THEN
              rde = grhode(a)
              x = ( rde - CP%omegaf*grhom ) / (CP%omegatr*grhom)
              w_de = p0 * ( tanh(x) - 1. ) / rde
!print*, 'w', w_de
           endif
...
!----------------------------
         function w_de_dot(a)
           ...
           adotoa = 1./(a*dtauda(a))
! --- Affine
           IF (CP%do_affine_eos) w_de_dot = 3. * adotoa * p0/grhode(a) * (1. + w_de(a) )

! --- Exp
           IF (CP%do_exp_eos) THEN 
              rde = grhode(a)
              x = (rde - grhol) / grhoctilde
              w_de_dot = grhol*exp(-x)*(1./grhoctilde + 1./rde)*(-3.*adotoa*(1.-grhol/rde*exp(-x)) )
!              w_de_dot = exp(-x)*(grhol/grhoctilde + grhol/rde)*(-3.*adotoa*(1.-grhol/rde*exp(-x)) )
!              w_de_dot = -3.*adotoa*(1.+w_de(a))*(adia_sound_speed(a)-w_de(a))
           ENDIF
...
! --- Tgh
           IF (CP%do_tgh_eos) THEN
              rde = grhode(a)
              x = ( rde - CP%omegaf*grhom ) / (CP%omegatr*grhom)
              w_de_dot = -3.*adotoa*( 1. + p0*(tanh(x)-1.)/rde ) * ( p0/(CP%omegatr*grhom) * (1.- tanh(x)**2) - p0 / rde * (tanh(x) - 1. )  )
!print*, 'wd', w_de_dot
              IF (abs(w_de_dot) .lt. 1.e-3) w_de_dot = 0.
           ENDIF

         end function w_de_dot
...
! --- Tgh
           IF (CP%do_tgh_eos) THEN
              rde = grhode(a)
              x = ( rde - CP%omegaf*grhom ) / (CP%omegatr*grhom)
              cs2_ad = p0/(CP%omegatr*grhom) * (1.-tanh(x)**2)
!print*, 'cs2', cs2_ad
              if (abs(cs2_ad) .lt. 1.e-3) cs2_ad = 0.
              if (abs(cs2_ad) .gt. 1.) cs2_ad = 1.
           ENDIF
...
!----------------------------
! --- Mar 2009
         SUBROUTINE Init_Rhooa(rho_a_out, already_done)
           
           USE ModelParams, ONLY: grhov, nstep_D, ailn, afln, lna_i, lna_f, dlna

           IMPLICIT NONE

!         type(EvolutionVars) EV

           REAL(dl), DIMENSION(nstep_D,3) :: rho_a_out
           REAL(dl) :: c_D(24),w_D(1,9), tol1_D, lna, lnaend, grhoDE0
           INTEGER :: ind_D
           INTEGER, OPTIONAL, INTENT(INOUT) :: already_done

! --- Inner variables
           REAL(dl) :: RhoVar,  lna0!lna_i, lna_f, dlna,
           INTEGER :: NoEqsToPropagate = 1, NoVar_D, i, istep0
...
!!$
!!$              WRITE(*,*) " Initialising background..."

              NoEqsToPropagate = 1
              NoVar_D = NoEqsToPropagate
              ...
              lna0 = 0.
              lna_i = ailn * LOG(10._dl)
              lna_f = afln * LOG(10._dl)
              dlna = (lna_f-lna_i) / (nstep_D-1)

              istep0 = int( abs(lna_i) / dlna )
!print*, istep0
!           print*, 'lna0_index = ', istep0
!           print*, ailn, afln

! --- lookback integration
              lna = lna0
              rho_a_out(istep0,2) = grhoDE0
              rho_a_out(istep0,1) = lna0
!count = 0
              DO i = 1, istep0-1
!count = count + 1
                 lnaend = lna0 - i*dlna 
                 rho_a_out(istep0-i,1) = lnaend
!              if (i==1) print*, '-->', lna, lnaend
                 call dverk(RhoVar, NoEqsToPropagate, grhoDEdotoa, lna, grhoDE0, lnaend, tol1_D, ind_D, c_D, NoVar_D, w_D)
                 rho_a_out(istep0-i,2) = grhoDE0
              ENDDO
!print*, count
!count = 0
              lna = 0.
              ind_D = 1
              grhoDE0 = grhov
              DO i = istep0+1, nstep_D
!count = count + 1
                 lnaend = lna0 + (i-istep0)*dlna
                 rho_a_out(i,1) = lnaend
!              if (i==nstep_D) print*, '-->', lna, lnaend
                 call dverk(RhoVar, NoEqsToPropagate, grhoDEdotoa, lna, grhoDE0, lnaend, tol1_D, ind_D, c_D, NoVar_D, w_D)
                 rho_a_out(i,2) = grhoDE0
              ENDDO
!print*, count
!pause
...
!!$           ELSE
!!$              WRITE(*,*) " Already initialised!"
!!$              RETURN
!!$           ENDIF

         END SUBROUTINE Init_Rhooa

!----------------------------
         SUBROUTINE grhoDEdotoa(RhoVar, Nvars, lna, grho_i, dgrhodlna )

           USE ModelParams!, ONLY: CP%omegal, CP%omegatr, omegactilde 

           IMPLICIT NONE

           REAL(dl), INTENT(IN) :: lna, RhoVar
           INTEGER, INTENT(IN)  :: Nvars
           REAL(dl), DIMENSION(Nvars), INTENT(OUT)  :: dgrhodlna
           REAL(dl), DIMENSION(Nvars), INTENT(INOUT)  :: grho_i
...
! ------ Exponential EoS
           IF (CP%do_exp_eos) THEN
              x = (grho_i(i_grho) - grhol) / grhoctilde
           
              dgrhodlna = -3._dl * (grho_i - grhol * EXP(-x))
...
! ------ Hyperbolic Tangent
           IF (CP%do_tgh_eos) THEN
              ...
              x = ( grho_i(i_grho) - CP%omegaf*grhom ) / (CP%omegatr * grhom)

              dgrhodlna = -3._dl * (grho_i + p0 * ( tanh(x)-1. ) )

           ENDIF

         END SUBROUTINE  grhoDEdotoa
...
! --- Mar 2009
       subroutine init_background
         ...
         USE ModelParams, ONLY: CP, grhoDE_lna, nstep_D, back_ini_status, grhol
         USE LambdaGeneral, ONLY: Init_Rhooa, grhoDEdotoa

         IMPLICIT NONE

         DOUBLE PRECISION, dimension(nstep_D) :: x, y, y2
         DOUBLE PRECISION :: yp1, ypn, dummy, dummyx
         DOUBLE PRECISION, DIMENSION(1) :: dummy1, dummyn, dummyy
         integer :: i
         double precision :: da, dtda
...
!!$         verbosity = .true.
         ...
         IF (CP%do_exp_eos .OR. CP%do_tgh_eos) THEN 
            ...
            CALL Init_Rhooa(grhoDE_lna, back_ini_status)

!         print*, 'pippo'
            IF (CP%do_exp_eos) OPEN(unit=34,file='exp_grhoDE_lna.dat',form='formatted',status='unknown',action='write')
!!$            IF (CP%do_tgh_eos) OPEN(unit=34,file='tgh_grhoDE_lna.unf',form='unformatted',status='unknown',action='write')
            IF (CP%do_tgh_eos) then
               OPEN(unit=34,file='tgh_grhoDE_lna.dat',form='formatted',status='unknown',action='write')
               OPEN(unit=35,file='tgh_grhoDE_DMonly_lna.dat',form='formatted',status='unknown',action='write')
            endif
            do i=1,nstep_D
               write(34,*) grhoDE_lna(i,:)
            enddo

            if (CP%do_tgh_eos) then
               do i=1, nstep_D
                  write(35,*) grhoDE_lna(i,1), grhoDE_lna(i,2)-grhol
               enddo
            endif
!         write(*,*) grhoDE_lna(10000,:)
            close(34)
            if (CP%do_tgh_eos) close(35)

            OPEN(unit=36,file='H_a.dat',form='formatted',status='unknown',action='write')
            da = 1./nstep_D
            do i=1, nstep_D
               write(36,*) da*i, 1./dtda(da*i)/(da*i)
            enddo
            close(36)

            if (verbosity) then
               print*, " Init_background: equations5.f90 - density and Hubble written on file."
               print*, CP%omegal, CP%omegatr, CP%omegaf
               verbosity = .false.
            endif

! ------ Interpolating DE density:
! --- 1st: calling spline to get the second derivative
            x = grhoDE_lna(:,1)
            y = grhoDE_lna(:,2)
            ...
            dummyy = y(1)
!         print*, dummyx, dummyy
            CALL grhoDEdotoa( dummy, 1, dummyx, dummyy, dummy1 )
            dummyx = x(nstep_D)
            dummyy = y(nstep_D)
!         print*, dummyx, dummyy
            CALL grhoDEdotoa( dummy, 1, dummyx, dummyy, dummyn )
...
!         print*, yp1, ypn
            ...
            CALL spline(x, y, nstep_D, yp1, ypn, y2)
            grhoDE_lna(:,3) = y2(:)

         ENDIF

!call cpu_time(t2)
!print*, 'init_back', t2-t1
...
!         end if
! ------ :DP
          if (CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_tgh_eos) then 
             grhoa2 = grhoa2 + grhode(a)*a2*a2
!print*, 'dtauda DE contr'
!print*, grhode(a)*a2*a2
!pause
          else
...
!print*, t2-t1
!pause
!!$          write(*,*) 'GaugeIntef_ScalEv',y(1),y(3),y(EV%w_ix),y(EV%w_ix+1)*EV%q
         ...
         !Not tested for non-adiabatic         
           ...
           if (.not. EV%no_rad_multpoles .and. tauend > tau_switch .and. DoLateRadTruncation &
              ...
              if (CP%Num_Nu_massive == 0) then
                 ...
                 if ( (w_lam /= -1 .and. w_Perturb) .or. CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_tgh_eos) then
!actually DE perturbations probably irrelevant and could set to zero too
                    y(6)=y(EV%w_ix)
                    y(7)=y(EV%w_ix+1)
                    EV%w_ix = 6
...
!            EV%nvar=EV%nvar+2
! ------ :DP
!              if ( (w_lam /= -1 .and. w_Perturb) .or. CP%do_affine_eos .OR. CP%do_exp_eos) then
! Mar 2009
              if ( (w_lam /= -1 .and. w_Perturb) .or. CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_tgh_eos) then
! ---
                 EV%w_ix = EV%nvar+1
...
! ------ :DP
! --- Dec 2009
! redshift distribution variables
        real(dl) zz!, zbar, znot
        integer ISWcat
! NVSS fitting formula Ho et al. 0801.0642
        real(dl) alpha, zstar, beff, nvss_wf, norm
! ---
        yprime = 0
        if (CP%flat) then
...
! ------ :DP
        if ( (.not. CP%do_affine_eos) .AND. (.not. CP%do_exp_eos) .AND. (.not. CP%do_tgh_eos) ) grhov_t = grhov*a**(-1-3*w_lam)
        if (CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_tgh_eos) grhov_t = grhode(a)*a2
! ---
...
! ------ :DP
        if ( (.not. CP%do_affine_eos) .AND. (.not. CP%do_exp_eos) .AND. (.not. CP%do_tgh_eos) ) gpres=(grhog_t+grhor_t)/3+grhov_t*w_lam
        if (CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_tgh_eos) gpres=(grhog_t+grhor_t)/3+grhov_t*w_de(a)
! ---
...
!  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
        ...
        if ((w_lam /= -1 .and. w_Perturb) .and. (.not. CP%do_affine_eos) .and. (.not. CP%do_exp_eos) .AND. (.not. CP%do_tgh_eos) ) then
           clxq=y(EV%w_ix)
           vq=y(EV%w_ix+1) 
...
! --- :DP
           ...
           beff    = 1.98
           norm = 1.18
           nvss_wf = alpha**(alpha+1.)/zstar**(alpha+1)/0.9237 * zz**alpha * exp(-alpha*zz/zstar) / norm
!           1.18**(1.18+1)/0.79**(1.18+1)/0.9237 * 1.98 * zz**(1.18) * exp(-1.18*zz/0.79)

           if (zz .LE. 100.) sources( isource_isw ) = ( 4.D0/3.D0*EV%Kf(1)*expmmu(j)*sigma + 2.D0/3.D0*(-sigma-t4*etak)*expmmu(j) )*k &
                -t4*expmmu(j)*dgrho/3.D0 &
                + ( &
!( 11.D0/10.D0*sigma - 3.D0/8.D0*EV%Kf(2)*ypol(3)+vb+ 3.D0/40.D0*qg-9.D0/80.D0*EV%Kf(2)*y(9) ) * dvis(j)
                + (5.D0/3.D0*grho+gpres)*sigma*expmmu(j) &
                +( -2.D0*adotoa*etak*expmmu(j) &
! + 21.D0/10.D0*etak*vis(j)
                ) / EV%Kf(1) &
!+( vbdot-3.D0/8.D0*EV%Kf(2)*ypolprime(3)+3.D0/40.D0*qgdot-21.D0/5.D0*sigma*adotoa-9.D0/80.D0*EV%Kf(2)*yprime(9) ) * vis(j) 
                ) / k &

                + &
                ( &
!( (-9.D0/160.D0*pigdot-27.D0/80.D0*ypolprime(2) )*opac(j)-21.D0/10.D0*dgpi -27.D0/80.D0*dopac(j)*ypol(2)-9.D0/160.D0*dopac(j)*pig )*vis(j) 
                - diff_rhopi*expmmu(j) & !+ 
!( (-27.D0/80.D0*ypol(2)-9.D0/160.D0*pig)*opac(j)+3.D0/16.D0*pigdot+9.D0/8.D0*ypolprime(2) )*dvis(j) + 
!9.D0/8.D0*ddvis(j)*ypol(2)+3.D0/16.D0*ddvis(j)*pig 
                ) / t92 !&
! wf applied to ISW too
!                * nvss_wf
           if (zz .GT. 100.) sources( isource_isw) = 0.
...
!            Arr(Transfer_tot) = dgrho/grho/k2 
                 ...
                 sources( Num_first_ISW + ISWcat - 1) = &
                      sqrt(2.) * pi * sqrt(CP%InitPower%ScalarPowerAmp(1)) * (grhoc_t/(grhoc_t+grhob_t)*clxc + grhob_t/(grhoc_t+grhob_t)*clxb) * adotoa/a * CP%norm_gw * exp( -( (zz-CP%znot)/CP%sigz )**2 )
              else
!!$                 sources( Num_first_ISW + ISWcat - 1) = &
!!$                      sqrt(2.) * pi * sqrt(CP%InitPower%ScalarPowerAmp(1)) * adotoa/a * (clxc + clxb)
! NVSS wf from Ho et al.

                 sources( Num_first_ISW + ISWcat - 1) = &
                      beff * nvss_wf * &
                      sqrt(2.) * pi * sqrt(CP%InitPower%ScalarPowerAmp(1)) * ( grhoc_t/(grhoc_t+grhob_t)*clxc + grhob_t/(grhoc_t+grhob_t)*clxb) * adotoa/a
!!$                 sources( Num_first_ISW + ISWcat - 1) = &
!!$                      nvss_wf * &
!!$                      sqrt(CP%InitPower%ScalarPowerAmp(1)) * (clxc + clxb) * adotoa/a
              endif
           end do
        end if
!ejv          
...
! The difference is tiny because deep in radiation epoch
        if (CP%omegac == 0.) om = (grhob+grhoc+grhomdf)/sqrt(3*(grhog+grhonu))       
! ---
        omtau=om*tau
...
! ------ :DP
        ...
        if (CP%omegac == 0.) Rc=CP%omegac+(CP%omegav-CP%omegal)/(CP%omegac+(CP%omegav-CP%omegal)+CP%omegab)
! ---
        Rb=1-Rc
        Rp15=4*Rv+15
...
!!$        write(*,*) 'initial', w_de(a)
        IF (CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_tgh_eos) initv(1,i_clxq) = ( 1.+w_de(a) ) * initv(1,i_clxb)
!        write(*,*) "initial de:" ,  initv(1,i_clxq)
! ---
...
!           write(*,*) "de: c, v",  y(EV%w_ix), y(EV%w_ix+1)
        end if
...
! ------ :DP
! If interested in tensor vector modes check omegac ==> omegac+omega_df
        implicit none
...
! dgrho is the density fluctuation
               dgrho = dgrho+(clxc*grhoc + clxb*grhob)/a + &
                    clxq*grhode(a)*a**2    
!!$            IF (CP%omegac == 0.) THEN
               IF ( CP%do_affine_eos ) grho = grho + (grhoc+grhob)/a + grhomdf*a**(-1-3*CP%alpha)!grhode(a)*a**2
               IF ( CP%do_exp_eos )    grho = grho + (grhoc+grhob)/a + (grhode(a)-grhol) * a**2
               IF ( CP%do_tgh_eos )    grho = grho + (grhoc+grhob)/a + (grhode(a)-grhol) * a**2

               Arr(Transfer_qm) = clxq * grhode(a) / (grhode(a)-grhol) /k2
...
! ------ :DP
         real(dl) :: rde, wde, wded, cad2, x, Dadotoa, dtauda
!real :: t1, t2
         external dtauda
         logical, save :: speak = .true.
! ---
              

!print*, 'fderivs - pincopallino'
!call cpu_time(t1)
...
! ===========================
! --- Mar 2009
! Definition of DE variables at the beginning to try to speed the code up
         IF (CP%do_exp_eos .OR. CP%do_tgh_eos) THEN
            rde = grhode(a)
!            x = (rde - grhol) / grhoctilde
!            wde = - grhol * EXP(-x) / rde
!            Dadotoa = 1./(a*dtauda(a))
!            wded = grhol*exp(-x)*(1./grhoctilde + 1./rde)*(-3.*Dadotoa*(1.-grhol/rde*exp(-x)) )
!            cad2 = grhol/grhoctilde * EXP(-x)
!            IF (CP%do_barotropic_sos) cs2_lam = cad2
         ENDIF
...
!!$         WRITE(*,*) "equations.f90, fderivs; w_lam = ", w_lam
         if (CP%do_affine_eos) then 
            grhov_t = grhode(a)*a2 
         elseif (CP%do_exp_eos .OR. CP%do_tgh_eos) THEN
            grhov_t = rde * a2
         ELSE
...
!            rde = grhode(a)
            ...
            wde = - grhol * EXP(-x) / rde
!            Dadotoa = 1./(a*dtauda(a))
            wded = grhol*exp(-x)*(1./grhoctilde + 1./rde)*(-3.*adotoa*(1.-grhol/rde*exp(-x)) )
            cad2 = grhol/grhoctilde * EXP(-x)
            IF (CP%do_barotropic_sos) cs2_lam = cad2
         ENDIF
         ...
         IF (CP%do_tgh_eos) THEN
            x = (rde - CP%omegaf*grhom) / (CP%omegatr*grhom)
            wde = p0 / rde * (tanh(x) -1.)
            wded = -3.*adotoa*(1.+p0*(tanh(x)-1.)/rde) * (p0/(CP%omegatr*grhom)*(1.-tanh(x)**2) - p0/rde*(tanh(x)-1.))
            cad2 = p0/(CP%omegatr*grhom) * (1.-tanh(x)**2)
            IF (CP%do_barotropic_sos) cs2_lam = cad2
            if (.true.) then 
               if (speak) then
!!$               print*, " Check speed of sound. threshold 1 NOT removed!"
                  print*, " Check speed of sound. threshold 1 removed!"
                  speak = .false.
               endif
            else
               if (speak) then
                  print*, " Check speed of sound. threshold 1 NOT removed!"
                  speak = .false.
                  IF (CP%do_barotropic_sos .AND. cad2 > 1.) cs2_lam = 1.            
               endif
            endif
         ENDIF
! ---
...
! have to get z from eta for numerical stability
         ...
         if ((w_lam /= -1 .and. w_Perturb) .and. (.not. CP%do_affine_eos) .and. (.not. CP%do_exp_eos) .AND. (.not. CP%do_tgh_eos) ) then
!            print*, w_lam,w_Perturb,CP%do_affine_eos,CP%do_exp_eos, CP%do_tgh_eos
...
!!$            WRITE(*,*) "equations.f90, fderivs; delta, v DE ",ayprime(EV%w_ix),ayprime(EV%w_ix)
         ...
         if (CP%do_affine_eos) then
! --- NonLinear
            ayprime(EV%w_ix) = -3*adotoa*(cs2_lam-w_de(a)) * (clxq + 3*adotoa*(1+w_de(a))*vq/k) &
                 -(1+w_de(a))*k*(vq+z) - 3._dl*adotoa*w_de_dot(a)*vq/k

!            ayprime(EV%w_ix+1) = 0.
!            if (w_de(a) /= -1.) ayprime(EV%w_ix+1) = -adotoa*( 1-3*cs2_lam )*vq + k*cs2_lam*clxq/(1+w_de(a))
            ayprime(EV%w_ix+1) = -adotoa*( 1-3*cs2_lam )*vq + k*cs2_lam*clxq/(1+w_de(a))
         endif

         if (CP%do_exp_eos .OR. CP%do_tgh_eos) then
! --- Exponential
            ayprime(EV%w_ix) = -3*adotoa*(cs2_lam-wde) * (clxq + 3*adotoa*(1+wde)*vq/k) &
                 -(1+wde)*k*(vq+z) - 3._dl*adotoa * wded * vq/k
...
! ------ :DP
            if ( (.not. CP%do_affine_eos) .AND. (.not. CP%do_exp_eos) .AND. (.not. CP%do_tgh_eos) ) gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_lam
!            if () gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_lam

            if (CP%do_affine_eos) gpres = gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_de(a)
            if (CP%do_exp_eos .OR. CP%do_tgh_eos)    gpres = gpres+ (grhog_t+grhor_t)/3 +grhov_t*wde
! ---
...
! ------ :DP
    if ( (.not. CP%do_affine_eos) .AND. (.not. CP%do_exp_eos) .AND. (.not. CP%do_tgh_eos) ) gpres=(grhog_t+grhor_t)/3._dl+grhov_t*w_lam 
    if (CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_tgh_eos) gpres=(grhog_t+grhor_t)/3._dl+grhov_t*w_de(a) 
! ---
...
! ------ :DP
        ...
        if (CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_tgh_eos) then
!!!$           ayprime(EV%w_ix) = -3*adotoa*(cs2_lam-w_de(a))*(clxq+3*adotoa*(1+w_de(a))*vq/k) &
!!$                -(1+w_de(a))*k*(vq+z) + w_de_dot(a)/(1+w_de(a))*clxq
!!$           ayprime(EV%w_ix+1) = -adotoa*(1-3*cs2_lam)*vq + k*cs2_lam*clxq/(1+w_de(a))
!_------------------------------------------------------------------------------------
            ayprime(EV%w_ix) = -3*adotoa*(cs2_lam-w_de(a)) * (clxq + 3*adotoa*(1+w_de(a))*vq/k) &
                 -(1+w_de(a))*k*(vq+z) - 3._dl*adotoa*w_de_dot(a)*vq/k
...
! ------ :DP
            if ( (.not. CP%do_affine_eos) .AND. (.not. CP%do_exp_eos) .AND. (.not. CP%do_tgh_eos) ) gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_lam
            if (CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_tgh_eos) gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_de(a)
! ---
