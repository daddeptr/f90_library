...
!! See equations4.f90 for additional information
!!
...
!        character(LEN=*), parameter :: version = 'Nov_06 - Modfied by D.Pietrobon Mar09 with O.Piattella'
        character(LEN=*), parameter :: version = 'Nov_06 - Further Modified by D.Pietrobon Dec09 after changes by DP Mar09 with OP'
...
!ejv
         logical  :: do_barotropic_sos, do_affine_eos, do_exp_eos, do_tgh_eos
         real(dl) :: omegal, alpha
! --- Mar 2009
         real(dl) :: omegatr
         real(dl) :: omegaf ! tgh, p0 * (tgh((omega-omegaf)/omegatr) -1)
! ---
...
! --- Dec 2009
!!$        integer  :: Num_ISW_catalogs
        integer  :: Num_first_ISW, isource_isw        
! ---
        real(dl) :: p0, grhol, grhomdf !(8piG rho_i*a**2) a=1 ==> 8piG rho_i0
! --- Mar 2009
        real(dl) :: omegactilde, grhoctilde
        INTEGER, PARAMETER :: nstep_D = 5000
        REAL(dl), DIMENSION(nstep_D,3) :: grhoDE_lna ! Three rows: lna, rho, d2rhoda2
!        REAL(dl), DIMENSION(0:nstep_D,2) :: wDE_lna ! Two rows: wDE, wDEdot
        REAL(dl), PARAMETER :: ailn = -8.
        REAL(dl), PARAMETER :: afln = 0.2
        REAL(dl) :: lna_i, lna_f, dlna
        INTEGER :: back_ini_status = 0
! ---
...
! ------ :D
             if (CP%do_affine_eos) then
                grhol = grhom * CP%omegal
                p0 = -grhol*( 1+CP%alpha )
                grhomdf = grhov - grhol
             endif
! --- Dec 2009
! moved in inidriver.F90
             if (CP%do_isw) then 
                print*, " ISW ccf on"
                if (CP%do_gw) then 
                   print*, " Gaussian CDM window function selected:"
                   write(*,140) P%norm_gw, " exp( -( (z-", P%znot, " ) / ", P%sigz, " )^2 )"
140                format(1f6.3,1a12,1f6.3,1a5,1f6.3,1a6)
                   print*, ""
                endif
             endif
! --- Mar 2009
             IF (CP%do_exp_eos) THEN
                omegactilde = (CP%omegatr-CP%omegal) / LOG(3._dl*CP%omegal/CP%omegatr)
                grhoctilde = grhom * omegactilde
                grhol = grhom * CP%omegal
                grhomdf = grhov - grhol
             ENDIF
! --- tgh EoS
             IF (CP%do_tgh_eos) THEN
                grhol = CP%omegal * grhom
                grhomdf = grhov - grhol
                p0 = -grhol / (tanh( (CP%omegal-CP%omegaf)/CP%omegatr )-1.)
                if (verbosity) then 
                   print*, "Omegal, omegatr, omegaf"
                   print*, P%omegal, P%omegatr, P%omegaf
                endif
             ENDIF
! ---
...
! ------ :D
! init_background used to initialise rho(a) for the exponential models
!call cpu_time(t2)
!print*,' cambparams_set: before init_back',t2-t1
!call cpu_time(t1)
            call init_background
!call cpu_time(t2)
!print*,' cambparams_set: after init_back',t2-t1
!call cpu_time(t1)
!print*,' cambparams_set: beofre timeofz'
! ---
            CP%tau0=TimeOfz(0._dl)
            last_tau0=CP%tau0
!call cpu_time(t2)
!print*,' cambparams_set: after timeofz',t2-t1
!call cpu_time(t1)
           else
              CP%tau0=last_tau0
           end if         
...
!print*, error    
           if (present(error)) then
              ...
              write(*,'("Om_Lambda            = ",e13.8)') CP%omegav
              write(*,'("Om_K                 = ",f7.5)') CP%omegak
! ------ :D           
             if (CP%do_affine_eos) then
                write(*,*) ""
                write(*,*) "Affine equation of state: parameters:"
                write(*,'("Omega_l              = ",f13.5)') CP%omegal
                write(*,'("alpha                = ",f13.5)') CP%alpha
                write(*,'("grhom*p0             = ",e13.5)') p0
                write(*,'("p0                   = ",e13.5)') p0/8./pi
                write(*,'("grhom                = ",e13.5)') grhom
                write(*,'("Delta                = ",f8.5)') 4*p0*0. - (1+CP%alpha)**2
                if (CP%do_barotropic_sos) write(*,*) "Barotropic fluid chosen"
                if (CP%do_barotropic_sos .eqv. .false.) write(*,*) "Effective sound speed chosen"
!!$                write(*,'("Sound speed          = ",f8.5)') cs2_lam
                write(*,*) ""
                if (CP%omegav < CP%omegal) then
                   write(*,*) ""
                   write(*,*) "Bad input parameters: Omegav < Omegal"
                   stop
                endif
! --- Mar 2009
             elseif (CP%do_exp_eos) THEN
                write(*,*) ""
                write(*,*) "Exponential EoS: parameters:"
                write(*,'("Omega_l              = ",f13.5)') CP%omegal
                write(*,'("Omega_tr             = ",f13.5)') CP%omegatr
!                write(*,'("grhom*p0             = ",e13.5)') p0
!                write(*,'("p0                   = ",e13.5)') p0/8./pi
                write(*,'("grhom                = ",e13.5)') grhom
                write(*,'("grhov                = ",e13.5)') grhov
                write(*,'("grhol                = ",e13.5)') grhol
!                write(*,'("Delta                = ",f8.5)') 4*p0*0. - (1+CP%alpha)**2
                if (CP%do_barotropic_sos) write(*,*) "Barotropic fluid chosen"
                if (CP%do_barotropic_sos .eqv. .false.) write(*,*) "Effective sound speed chosen"       
             ...
             elseif (CP%do_tgh_eos) THEN
                write(*,*) ""
                write(*,*) "Hyperbolic tangent EoS: parameters:"
                write(*,'("Omega_f              = ",f13.5)') CP%omegaf
                write(*,'("Omega_tr             = ",f13.5)') CP%omegatr
                write(*,'("Omega_l              = ",e13.5)') CP%omegal
                write(*,'("p0                   = ",e13.5)') p0
!!$                write(*,'("grhom                = ",e13.5)') grhom
!!$                write(*,'("grhov                = ",e13.5)') grhov
                write(*,'("grhol                = ",e13.5)') grhol
...
!call cpu_time(t2)
!print*,'after my initialization ',t2-t1
!print*,'before rofchi'
!call cpu_time(t1)
           CP%chi0=rofChi(CP%tau0/CP%r)
!call cpu_time(t2)
!print*,'after rofchi', t2-t1
...
!         allocate(Cl_scalar(lmin:CP%Max_l, CP%InitPower%nn, C_Temp:C_last+2*CP%Num_ISW_catalogs))
         allocate(Cl_scalar(lmin:CP%Max_l, CP%InitPower%nn, C_Temp:C_last+2*CP%Num_ISW_catalogs+1)) !isw-isw added
! ---
         Cl_scalar = 0
        end if
...
! --- !!! To be modified if CP%Num_ISW_catalogs > 1
!                    write(fileio_unit,'(1I5,5E15.5)')il ,fact*Cl_scalar(il,in,C_Temp:C_last+2*CP%Num_ISW_catalogs)
                    write(fileio_unit,'(1I5,6E15.5)')il ,fact*Cl_scalar(il,in,C_Temp:C_last+2*CP%Num_ISW_catalogs+1) ! isw-isw added
                 else
                    write(fileio_unit,'(1I5,3E15.5)')il ,fact*Cl_scalar(il,in,C_Temp:C_Cross)
                 endif
! ---
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        integer, parameter :: Transfer_kh =1, Transfer_cdm=2,Transfer_b=3,Transfer_g=4, &
                              ...
                              Transfer_tot=7, &
! ------ :D
                              Transfer_q = 8, Transfer_qm = 9
       
!        integer, parameter :: Transfer_max = Transfer_tot
        integer, parameter :: Transfer_max = Transfer_tot+2
! ---
...
! ------ :D
          character(len=2) :: io_Tmax
! ---
          write(io_Tmax,'(1I2)') Transfer_max
...
! ------ :D
                 write(fileio_unit,'('//trim(adjustl(io_Tmax))//'E14.6)') MTrans%TransferData(Transfer_kh:Transfer_max,ik,i)
! ---
                end if
             end do
            close(fileio_unit)
...
! ------ :D
! Testing if omegac ==> omegac+omegadf
! useless
        a_verydom = AccuracyBoost*10*(grhog+grhornomass)/(grhomdf+grhoc+grhob)
! ---
...
! ------ :D
! Testing if it is necessary omegac ==> omegac + omega_df
!           useless!!   
           ( ( CP%omegav-CP%omegal ) +CP%omegac+CP%omegab)/CP%h0**2))  
! ---
              iv=2
           end if
         end do
