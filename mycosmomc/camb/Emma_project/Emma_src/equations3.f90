...
!print*, 'cs2', cs2_ad
print*, " No constraints on sound speed!"
...
! ------ :D
        if (CP%do_affine_eos .eqv. .false. .AND. CP%do_exp_eos .eqv. .false. .AND. CP%do_tgh_eos .eqv. .false.) grhov_t = grhov*a**(-1-3*w_lam)
...
! ------ :D
        if (CP%do_affine_eos .eqv. .false. .AND. CP%do_exp_eos .eqv. .false. .AND. CP%do_tgh_eos .eqv. .false.) gpres=(grhog_t+grhor_t)/3+grhov_t*w_lam
...
!  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
        ...
        if ( (w_lam /= -1 .and. w_Perturb) .and. (CP%do_affine_eos .eqv. .false.) .and. & 
             (CP%do_exp_eos .eqv. .false.) .AND. (CP%do_tgh_eos .eqv. .false.) ) then
           clxq=y(EV%w_ix)
           vq=y(EV%w_ix+1) 
...
! ---
         ...
         k2   = EV%k2_buf
! ------ :D
         IF ( (w_lam /= -1 .and. w_Perturb) .or. CP%do_affine_eos .OR. CP%do_exp_eos .OR. CP%do_tgh_eos) clxq = y(EV%w_ix)
! ---
...
! ------ :D
            if (CP%do_affine_eos .eqv. .false. .AND. CP%do_exp_eos .eqv. .false. .AND. CP%do_tgh_eos .eqv. .false.) gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_lam
...
! ------ :D
    if (CP%do_affine_eos .eqv. .false. .AND. CP%do_exp_eos .eqv. .false. .AND. CP%do_tgh_eos .eqv. .false.) gpres=(grhog_t+grhor_t)/3._dl+grhov_t*w_lam 
...
!  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
        ...
        if (w_lam /= -1 .and. w_Perturb .and. CP%do_affine_eos .eqv. .false. .and. CP%do_exp_eos .eqv. .false. .AND. CP%do_tgh_eos .eqv. .false.) then
           clxq=ay(EV%w_ix)
           vq=ay(EV%w_ix+1) 
           dgrho=dgrho + clxq*grhov_t
           dgq = dgq + vq*grhov_t*(1+w_lam)
